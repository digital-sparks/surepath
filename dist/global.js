(() => {
  ...
  window.Webflow.push(() => {
    ...
    gsapWithCSS.set(nav, { backgroundColor: "rgba(255,255,255,0)" });
    gsapWithCSS.set(navBackground, { opacity: 0 });
    if (navState)
    ...
    ScrollTrigger2.create({
      ...
      onEnter: () => {
        gsapWithCSS.to(nav, { backgroundColor: "rgba(255, 255, 255, 1)", duration: animationDuration });
        if (navState) {
          navIsInversed = false;
      ...
      onLeaveBack: () => {
        gsapWithCSS.to(nav, { backgroundColor: "rgba(255, 255, 255, 0)", duration: animationDuration });
        if (navState) {
          navIsInversed = true;
          nav.classList.add("inverse");
        }
    ...
    document.querySelectorAll(".nav_dropdown").forEach((dropdown) => {
      dropdown.addEventListener("mouseenter", () => {
        gsapWithCSS.to(navBackground, { opacity: navState ? 0.3 : 0, duration: animationDuration });
      });
      dropdown.addEventListener("mouseleave", () => {
        gsapWithCSS.to(navBackground, { opacity: 0, duration: animationDuration });
      });
    });
    if (navState) {
